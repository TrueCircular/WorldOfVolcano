...
EffectOutput StormVS(EffectMesh input)
{
    ...
    float multime = 1;//    +time * 3;

    output.position = input.position;
    output.position.x = output.position.x * multime;
    output.position.y = output.position.y * multime;
...
float4 PS(EffectOutput input) : SV_TARGET
{
    ...
    float ubasetime = time*LowParticleColor.r;
    float vBasetime = time * LowParticleColor.g;
